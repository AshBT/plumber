# Link Plumber [![Build Status](https://travis-ci.org/qadium/plumber.svg)](https://travis-ci.org/qadium/plumber) [![Coverage Status](https://coveralls.io/repos/qadium/plumber/badge.svg)](https://coveralls.io/r/qadium/plumber)
...
Once we've ingested the memex_ht database into neo4j, we only have the ad records and nothing more. To enhance the data and construct entities, run
    ...
    ./plumber -v Attributer Imager BaseCreator ImageLinker Twitter Youtube Instagram FaceFinder TextHash TextHashLinker

This will run the `Attributer`, `Imager`, `BaseCreator`, `ImageLinker`, `Twitter`, `Youtube`, `Instagram`, `FaceFinder`, `TextHash`, `TextHashLinker` enhancers (in that order). The `-v` flag tells the script to be verbose; you can duplicate the flag for more debugging info, for instance: `./plumber -vvv` will show a lot of debugging information.
...
### Available enhancers
Enhancers are added to the `plugins` folder and made available to the `enhance` script. Running `./plumber -h` will show a list of available enhancers. Note that the enhancer names are *case sensitive*.
